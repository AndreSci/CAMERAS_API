                ...
                while self.allow_read_frame.get():
                    ...
                    if capture.isOpened():
                        ...
                        if self.do_frame.get() and ret:
                            ...
                            time.sleep(0.005)  # TODO продолжаем тест, возможно слишком долгий time.sleep() вызывает переполнение буфера видео-потока
                            frame_fail_cnt += 1
                    ...
                    # time.sleep(0.005)  # TODO продолжаем тесты (без time.sleep() opencv одна из нескольких камер падает исключением)
                    # Возможно time.sleep() работает как метод освобождения очереди для обработки другим потокам ...
